...
func FindUsers(ctx *gin.Context) {
	...
	logrus.Debugf("users displayed")
}

// Create User using Create()
func CreateUser(ctx *gin.Context) {
	//validate User
	logrus.Debugf("data is validating")
...
func FindUser(ctx *gin.Context) {
	...
	logrus.WithField("id", ctx.Param("id")).Debug("user is getting searched")
	if err := models.DB.Where("id = ?", ctx.Param("id")).First(&user).Error; err != nil {
		logrus.WithFields(logrus.Fields{
			"error": err,
			"id":    ctx.Param("id"),
		}).Errorf("Failed to find the user with id %s", ctx.Param("id"))
		return
	}
	ctx.JSON(http.StatusOK, gin.H{"data": user})
	logrus.WithFields(logrus.Fields{
		"id": ctx.Param("id"), "user": user,
	}).Debug("user displayed")
}

//Update the User - PATCH
func UpdateUser(ctx *gin.Context) {
	//get model if exist
	logrus.Debugf("users is being searched")
	var user models.User
	if err := models.DB.Where("id = ?", ctx.Param("id")).First(&user).Error; err != nil {
		logrus.WithFields(logrus.Fields{
			"error": err,
	...
	ctx.JSON(http.StatusOK, gin.H{"data": user})
	logrus.WithField("id", ctx.Param("id")).Debug("user is updated")
	logrus.WithFields(logrus.Fields{
		"id": ctx.Param("id"), "user": user,
	}).Debug("user updated")
}
...
func DeleteUser(ctx *gin.Context) {
	...
	logrus.WithField("id", ctx.Param("id")).Debug("user deleted")
}
